...
int  main( int argc , char** argv) {
    ...
    magmaDoubleComplex     *a, *r; // a, r - nxn  matrices  on the  hostd
    ...
    magma_zmalloc_cpu (&h_work ,lwork );     //  memory  for  workspace//  Randomize  the  matrix a and  copy a -> r
    magma_dmalloc_cpu(&rwork, lrwork);
    lapackf77_zlarnv (&ione ,ISEED ,&n2 ,a);
    lapackf77_zlacpy(MagmaFullStr ,&n,&n,a,&n,r,&n);
    magma_zsetmatrix( n, n, a, n, d_r ,n,queue); // copy a -> d_r//  compute  the  eigenvalues   and  eigenvectors  for a symmetric ,// real  nxn  matrix; Magma  version
    ...
    printf("dsyevd  cpu  time: %7.5f sec.\n",cpu_time );   //  Lapack//  difference  in   eigenvalues
    //time
    blasf77_daxpy( &n, &mione , w1, &incr , w2, &incr);error = lapackf77_dlange( "M", &n, &ione , w2, &n, work );
