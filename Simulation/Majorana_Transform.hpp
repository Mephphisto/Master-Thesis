...
namespace mjize {
    // exp(i*theta) = cos(theta) + i*sin(theta) — avoids std::exp(complex<Dual>)
    // which libc++ cannot resolve for autodiff::dual components.
    inline std::complex<autodiff::dual> exp_i(const autodiff::dual& theta) {
        return std::complex<autodiff::dual>(autodiff::forward::cos(theta),
                                            autodiff::forward::sin(theta));
    }
    ...
    autodiff::dual opt_fnd(const autodiff::VectorXdual &x) {
        ...
        A = exp_i(x[4]) * ((A - Id) * (Id + A).inverse()).eval();
    ...
    autodiff::dual opt_fnd4(const autodiff::VectorXdual &x) {
        ...
        //@formatter:on
        A = exp_i(x[16]) * ((A - Id4) * (Id4 + A).inverse()).eval();
